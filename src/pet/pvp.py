...
class PVP(ABC):
    ...
    def encode(self, example: InputExample, priming: bool = False, labeled: bool = False) \
        ...
        parts_a = [(tokenizer.encode(x, add_special_tokens=False, max_length=512, truncation=True, **kwargs), s) for x, s in parts_a if x]

        if parts_b:
            parts_b = [x if isinstance(x, tuple) else (x, False) for x in parts_b]
            parts_b = [(tokenizer.encode(x, add_special_tokens=False, max_length=512, truncation=True, **kwargs), s) for x, s in parts_b if x]
...
class YelpPolarityPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 0:
            ...
            return [text, 'Taking everything into account, it was', self.mask, '.'], []
        elif self.pattern_id == 5:
            return [text, 'To wrap it up, it was', self.mask, '.'], []
        elif self.pattern_id == 6:
            return [text, 'Generally, it seems', self.mask, '.'], []
        elif self.pattern_id == 7:
            return [text, 'In a nutshell', self.mask, '.'], []
        elif self.pattern_id == 8:
            return ['The place had a', self.mask, 'vibe', '.', text], []
        elif self.pattern_id == 9:
            return ['Everything gave off a', self.mask, 'impression', '.', text], []
        elif self.pattern_id == 10:
            return [text, 'Broadly speaking, it appears', self.mask, '.'], []
        elif self.pattern_id == 11:
            return [text, 'Summing it all up, the restaurant appears', self.mask, '.'], []
        elif self.pattern_id == 12:
            return [text, 'To conclude, the feeling is', self.mask, '.'], []
        else:
            raise ValueError("No pattern implemented for id {}".format(self.pattern_id))
